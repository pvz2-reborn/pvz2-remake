...
public class levelSystem : MonoBehaviour
{
    ...
    Vector3 levelIconOffset = new Vector3(0f, -1.7f, 0f);
    ...
    private void creatLevelPosList () {
        /*
    ...
    private void creatLevelIcon () {
        ...
        for (int i = 0; i < wordMapMain.mapPieces.Count; i ++) {
            ...
            if(wordMapMain.mapPieces[i].eventType == "level") {
                ...
                Vector3 temp = levelName.transform.position + levelIconOffset; 
                //Debug.Log(temp);
                ...
                //存入关卡总列表
                wordMapMain.mapPieces[i].gameObject = levelObject;
    ...
    public void updateLevelIcon () {//todo 添加迷你游戏与危险关卡图标的支持
        ...
        for (int i = 0; i <= worldMap.playingLevel; i++) {
            Animator animator;
    ...
    public void levelFinished () {
        ...
        if(finishedLevel == worldMap.playingLevel) {//通过新关卡
            ...
            //更新mapPath
            animatorMapPath = mapPathList[finishedLevel].GetComponentInChildren<Animator>();
            animatorMapPath.Play("beamPathOpen");
            
            if (worldMap.playingLevel < levelList.Count) {//当下面还有关卡时 更新动画
    ...
    public void updataMapPath () {
        if (levelnum <= 1) {
            return;
        }

        for (int i = 1; i <= worldMap.playingLevel - 1; i ++) {
            Animator animator;
